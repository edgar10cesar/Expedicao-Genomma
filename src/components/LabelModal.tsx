...
export default function LabelModal({ pallet, onClose, palletNumber = 1 }: LabelModalProps) {
  ...
  const stickerContent = (isForPrint: boolean) => {
    return (
      <div 
        id={isForPrint ? "print-only-sticker" : "printable-pallet-sticker"}
        className={isForPrint ? "hidden print:flex bg-white border-2 border-black p-4 text-black text-left relative flex-col justify-between font-sans leading-tight select-none" : "bg-white border-2 border-black p-4 text-black text-left relative shadow-lg flex flex-col justify-between font-sans leading-tight select-none"}
        style={isForPrint ? {
          width: '100mm',
          height: '80mm',
      ...
      >
        {/* Header section */}
        <div className="flex justify-between items-start border-b-2 border-black pb-1.5" style={{ borderColor: '#000000' }}>
          <div>
            <strong className="text-md uppercase tracking-tight block" style={{ color: '#000000' }}>GENOMMA LAB.</strong>
            <span className="text-[9px] font-mono tracking-wider block" style={{ color: '#000000' }}>PLANO LOGÍSTICO DE EXPEDIÇÃO</span>
          </div>
          <div className="text-right">
            <span className="text-[8px] font-mono uppercase block text-right" style={{ color: '#000000' }}>ETIQUETA DE PALETE</span>
            <strong className="text-xs font-mono block text-right font-black" style={{ color: '#000000' }}>{pallet.id}</strong>
          </div>
        </div>
        ...
        <div className="grid grid-cols-12 gap-3 my-2.5 items-center flex-1">
          ...
          {/* QR Code Container */}
          <div className="col-span-5 flex flex-col items-center justify-center border-r border-dashed border-black pr-1.5 h-full" style={{ borderColor: '#000000' }}>
            <div className="bg-white p-1 border border-black rounded-xs" style={{ borderColor: '#000000' }}>
              <QRCodeSVG 
                value={qrPayload}
                size={isForPrint ? 90 : 110}
                level="M" 
                includeMargin={false}
              />
            </div>
            <span className="text-[8px] font-mono mt-1 text-center font-bold" style={{ color: '#000000' }}>SCAN ME TO LOAD</span>
          </div>

          {/* Grouped Shipments list (Up to 5) */}
          <div className="col-span-7 pl-1 flex flex-col justify-between h-full">
            <div className="mb-1 shrink-0">
              <span className="text-[8px] uppercase tracking-wider font-mono font-bold block mb-0.5" style={{ color: '#000000' }}>
                EMBARQUES CONSOLIDADOS (MAX 5)
              </span>
              <div 
                className="border-2 border-black py-1 flex items-center justify-center font-sans rounded"
                style={{ backgroundColor: '#ffffff', color: '#000000', borderColor: '#000000' }}
              >
                <span className="text-xs font-black font-mono tracking-widest" style={{ color: '#000000', fontWeight: '900' }}>
                  PALLET {palletNumber}
                </span>
              </div>
            </div>
              ...
              {pallet.shipments.map((sh, idx) => (
                ...
                >
                  <div className="truncate flex-1 min-w-0 pr-2">
                    <strong className="font-mono text-[10px] text-black pr-1 font-bold" style={{ color: '#000000' }}>{idx + 1}. {sh.shipmentNumber}</strong>
                    <span className="text-[9.5px] font-sans text-slate-800 font-semibold uppercase" style={{ color: '#000000' }}>{sh.clientName}</span>
                  </div>
                  <div className="text-right shrink-0">
                    <span 
                      className="font-mono font-bold text-black bg-slate-100 px-1 py-0.5 rounded border border-slate-250 text-[9px]"
                      style={{ backgroundColor: '#f1f5f9', color: '#000000', borderColor: '#cbd5e1' }}
                    >
                      {sh.volumes} Vol
                    </span>
                  </div>
        ...
        <div className="border-t-2 border-black pt-1.5 mt-auto flex justify-between items-end text-[9px] leading-tight font-mono" style={{ borderColor: '#000000' }}>
          <div className="flex-1 min-w-0 pr-2">
            <span style={{ color: '#000000' }}>CARREGAMENTO / TRANSPORTADORA:</span>
            <strong className="block text-[10px] font-sans truncate uppercase font-bold text-black flex items-center gap-1.5" style={{ color: '#000000' }}>
              <span>{pallet.carregamentoId.startsWith('carr_') ? 'ID Cópia: ' + pallet.carregamentoId.substring(5, 12) : pallet.carregamentoId}</span>
              <span 
                className="text-blue-700 font-extrabold tracking-tight bg-blue-50 px-1.5 py-0.5 rounded border border-blue-100 font-sans"
          ...
          <div className="text-right">
            <span style={{ color: '#000000' }}>GERADO EM:</span>
            <strong className="block font-bold" style={{ color: '#000000' }}>
